...
public class TravelPeopleService{
    ...
    public void addTravelPeople(TravelPeople travelPeople) {
        boolean flag = this.travelPeoplemapper.insert(travelPeople) == 1;
        if(flag){
    ...
    public void deleteTravelPeople(Integer id) {
        boolean flag = this.travelPeoplemapper.deleteByPrimaryKey(id) == 1;
        if(flag){
             throw new CommonException(ExceptionEnum.HOUSE_LISTING_BE_REPEAT);
        }
    }
    ...
    public void updateTravelPeople(TravelPeople travelPeople) {
        boolean flag =this.travelPeoplemapper.updateByPrimaryKeySelective(travelPeople) == 1;
        if(flag){
             throw new CommonException(ExceptionEnum.HOUSE_LISTING_BE_REPEAT);
        }
    }
