...
public class UserSearchHistoryService{
    ...
    public void addUserSearchHistory(UserSearchHistory userSearchHistory) {
        boolean flag = this.userSearchHistorymapper.insert(userSearchHistory) == 1;
        if(flag){
    ...
    public void deleteUserSearchHistory(Integer id) {
        boolean flag = this.userSearchHistorymapper.deleteByPrimaryKey(id) == 1;
        if(flag){
             throw new CommonException(ExceptionEnum.HOUSE_LISTING_BE_REPEAT);
        }
    }
    ...
    public void updateUserSearchHistory(UserSearchHistory userSearchHistory) {
        boolean flag =this.userSearchHistorymapper.updateByPrimaryKeySelective(userSearchHistory) == 1;
        if(flag){
             throw new CommonException(ExceptionEnum.HOUSE_LISTING_BE_REPEAT);
        }
    }
