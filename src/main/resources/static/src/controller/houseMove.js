layui.define(['table', 'form'], function (exports) {
    ...
    //++++++++++监听工具条操作开始++++++++++
    table.on('tool(LAY-houseMove-list)', function (obj) {//表格的名称
        var data = obj.data;
        if (obj.event === 'edit') {//匹配工具栏的edit字段
            admin.popup({
                ...
                , success: function (layero, index) {
                    view(this.id).render('houseMove/form', data).done(function () {//跳转的路径
                        form.render(null, 'houseMove-form');//读取表单的信息
                        //监听提交
                        form.on('submit(houseMove-form-submit)', function (data) {//form 表单提交的按钮
                            ...
                            $.ajax({
                                ...
                                url: "/house/move/update", // 请求地址
                                data: JSON.stringify(field), //请求附带参数
                                success: function () {
                                    layui.table.reload('LAY-houseMove-list'); //重载表格
    ...
    var active = {
        add: function () {
            admin.popup({
                ...
                , success: function (layero, index) {
                    view(this.id).render('houseMove/form').done(function () {
                        //监听提交
                        form.on('submit(houseMove-form-submit)', function (data) {
                            var field = data.field; //获取提交的字段
                            console.log(field)
                            $.ajax({
                                type: "POST", //请求方式 post
    ...
    $('.layui-btn.houseMove-form').on('click', function() {var type = $(this).data('type');
        active[type] ? active[type].call(this) : '';
    });
    //**********新增结束**********

    //==========搜索开始==========
    form.render(null, 'lay-admin-houseMove-form');
    form.on('submit(LAY-houseMove-back-search)',
        function(data) {
            var field = data.field;
            console.log(field)
            //执行重载
            table.reload('LAY-houseMove-list', {
                method: "post",
                url: "/house/move/search",
                where: field
            });
        });
    //==========搜索结束==========
