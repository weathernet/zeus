layui.define(['table', 'form'], function (exports) {
    ...
    //++++++++++监听工具条操作开始++++++++++
    table.on('tool(LAY-repairOrder-list)', function (obj) {//表格的名称
        var data = obj.data;
        if (obj.event === 'edit') {//匹配工具栏的edit字段
            admin.popup({
                ...
                , success: function (layero, index) {
                    view(this.id).render('repairOrder/form', data).done(function () {//跳转的路径
                        form.render(null, 'repairOrder-form');//读取表单的信息
                        //监听提交
                        form.on('submit(repairOrder-form-submit)', function (data) {//form 表单提交的按钮
                            ...
                            $.ajax({
                                ...
                                url: "/repair/order/update", // 请求地址
                                data: JSON.stringify(field), //请求附带参数
                                success: function () {
                                    layui.table.reload('LAY-repairOrder-list'); //重载表格
    ...
    var active = {
        add: function () {
            admin.popup({
                ...
                , success: function (layero, index) {
                    view(this.id).render('repairOrder/form').done(function () {
                        //监听提交
                        form.on('submit(repairOrder-form-submit)', function (data) {
                            var field = data.field; //获取提交的字段
                            console.log(field)
                            $.ajax({
                                type: "POST", //请求方式 post
    ...
    form.on('submit(LAY-repairOrder-back-search)',
        function(data) {
            ...
            console.log(field)
            //执行重载
            table.reload('LAY-repairOrder-list', {
                method: "post",
                url: "/repair/order/search",
                where: field
            });
        });
    //==========搜索结束==========
