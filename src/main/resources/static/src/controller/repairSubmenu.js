layui.define(['table', 'form'], function (exports) {
    ...
    //**********表格显示开始**********
    table.render({
        elem: '#LAY-repairSubmenu-list'
        , url: '/repair/submenu/query'
        , cols: [[
            {field: 'repairSubName', title: '子标题'}
            , {field: 'repairSubImage', title: '字图片', templet: '#Img'}
                    ...
                    view(this.id).render('repairSubmenu/form', data).done(function () {//跳转的路径
                        ...
                        form.on('submit(repairSubmenu-form-submit)', function (data) {//form 表单提交的按钮
                            ...
                            $.ajax({
                                ...
                                url: "/repair/submenu/update", // 请求地址
                                data: JSON.stringify(field), //请求附带参数
                                success: function () {
                                    layui.table.reload('LAY-repairSubmenu-list'); //重载表格
    ...
    var active = {
        add: function () {
            admin.popup({
                ...
                , success: function (layero, index) {
                    view(this.id).render('repairSubmenu/form').done(function () {
                        //监听提交
                        form.on('submit(repairSubmenu-form-submit)', function (data) {
                            var field = data.field; //获取提交的字段
                            console.log(field)
                            $.ajax({
                                type: "POST", //请求方式 post
    ...
    $('.layui-btn.repairSubmenu-form').on('click', function () {
    ...
    //==========搜索开始==========
    form.render(null, 'lay-admin-repairSubmenu-form');
    form.on('submit(LAY-repairSubmenu-back-search)',
        function (data) {
            var field = data.field;
            console.log(field)
            //执行重载
            table.reload('LAY-repairSubmenu-list', {
                method: "post",
                url: "/repair/submenu/search",
                where: field
            });
        });
    //==========搜索结束==========
