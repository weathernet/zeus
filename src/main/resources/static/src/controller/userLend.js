layui.define(['table', 'form'], function (exports) {
    ...
    //++++++++++监听工具条操作开始++++++++++
    table.on('tool(LAY-userLend-list)', function (obj) {//表格的名称
        var data = obj.data;
        if (obj.event === 'edit') {//匹配工具栏的edit字段
                    ...
                    view(this.id).render('userLend/form', data).done(function () {//跳转的路径
                        ...
                        form.on('submit(userLend-form-submit)', function (data) {//form 表单提交的按钮
                            ...
                            $.ajax({
                                ...
                                url: "/user/lend/update", // 请求地址
                                data: JSON.stringify(field), //请求附带参数
                                success: function () {
                                    layui.table.reload('LAY-userLend-list'); //重载表格
            ...
            layer.confirm('确定删除词条信息？', function (index) {
                ...
                $.post("/user/lend/delete", {id: id}, function (data) {
                    obj.del();
                    layer.close(index);
                })
            });
    ...
    var active = {
        add: function () {
            admin.popup({
                ...
                , success: function (layero, index) {
                    view(this.id).render('userLend/form').done(function () {
                        //监听提交
                        form.on('submit(userLend-form-submit)', function (data) {
                            var field = data.field; //获取提交的字段
                            console.log(field)
                            $.ajax({
                                type: "POST", //请求方式 post
    ...
    $('.layui-btn.userLend-form').on('click', function() {var type = $(this).data('type');
        active[type] ? active[type].call(this) : '';
    });
    //**********新增结束**********

    //==========搜索开始==========
    form.render(null, 'lay-admin-userLend-form');
    form.on('submit(LAY-userLend-back-search)',
        function(data) {
            var field = data.field;
            console.log(field)
            //执行重载
            table.reload('LAY-userLend-list', {
                method: "post",
                url: "/user/lend/search",
                where: field
            });
        });
    //==========搜索结束==========
